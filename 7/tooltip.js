...
function hideTip () {  														//this function hides the tool tip
	...
	sidTip.style.display ="none";											//first, get access to the tool tip HTML element																						//then, hide the tool tip HTML element by changing its CSS property "display" to "none"
}

function init () {																	/* the init function links functions to appropriate events of corresponding HTML elements */
	var sid = document.getElementById("sid");
	sid.onmouseover = showTip;																						/* create a variable named sid */
	sid.onmouseout = hideTip;																						/* get access to the HTML element by its id "sid" and link it to sid */
	sid.onfocus = showTip;
	sid.onblur = hideTip;
